import { logo, robotFace, robotHand } from "../assets";
import { QrCode } from "lucide-react";
import styles from "../styles";

export function HeroGiftCard() {
  return (
    <div className="flex flex-col">
      <div className="h-[500px] w-[400px] bg-gray-400 rounded-xl bg-clip-padding backdrop-filter backdrop-blur-md bg-opacity-20 p-8 z-20">
        ...
        <div className="bg-slate-500 rounded-md flex justify-between items-start p-6 mt-8 transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-110 duration-300 cursor-pointer">
          <div className="flex flex-1 flex-col max-w-[300px]">
            <h2 className="font-bold text-purple-700">KZ9TFGDE649835</h2>
            <h2 className="font-bold">Gift Card</h2>
            <p className="text-xl mt-2">
              O mundo dos cartões pré pagos evoluiu bastante nas últimas
              décadas, abrindo espaço para opções mais flexíveis. Entre essas
              inovações, os vale presente também conhecidos como cartão
              presente, se destacam como uma alternativa versátil e prática aos
      ...
      <img
        src={robotHand}
        className="hidden sm:block h-[360px] absolute top-[292px] right-0"
        alt=""
      />
    </div>
  );
